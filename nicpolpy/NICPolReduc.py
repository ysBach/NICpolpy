...
class NICPolReduc(NICPolReducMixin):
    ...
    # ):
    #     """
    #     If the user wants to COMBINE fringe frames, they must change `FRINCID`.
    #     Rows with the same FRINCID will be combined in `comb_frin`.
    #     FRINCID will also be used to name the output file.
    #     """
    #     self._check_skip()
    #     if self._skipexred["mfrin"]:
    #         if self.verbose:
    #             print("Summary exists. Skipping...")
    #         return self._read_summ("mfrin")

    #     plan = self._read_plan("mfrin")
    #     plan["FRINCID"] = plan["FILTER"].str.cat([plan["SETID"],
    #                                              plan["POL-AGL1"].map("{:04.1f}".format),
    #                                              plan["OERAY"]],
    #                                             sep="_")
    #     plan.drop("SETID", axis=1, inplace=True)
    ...
    #             fpath = Path(row["file"])
    #             ccd = load_ccd(fpath)
    #             ccd.header["FRINCID"] = (combid, "Mater fringe Combine-ID")
    #             if row["do_crrej"] > 0:
    #                 ccd, _ = crrej(
    #                     ccd,  # NOTE: Already FIXPIX'ed, so no need to use mask again here.
    ...
    #                     verbose=self.verbose >= 2
    #                 )
    #             _save(ccd, self.dirs["frin4"], _set_fstem_proc(fpath, ccd))
    #             plan_i = summary_nic(
    #                 self.dirs["frin4"],
    #                 **MATCHER["frin"]
    #             )[["file", "DATE-OBS"] + GROUPER["frin"] + ["FRINCID"]]
    #         _save_thumb(plan_i, self.plan_thumb_dirs["mfrin"],
    #                     show_progress=False, verbose=self.verbose, **thumb_kw)
    #     df_frins = make_summary(
    #         frins,
    #         keywords=["FRINCID"],
    #     )
    ...
    #         group_key="FRINCID",
    #         fmt="mfrin_{:s}",
    #         outdir=self.dirs["mfrin"],
    #     )
    #     return self._save_summ("mfrin", add_keys=["OERAY"])

    def step12_lv4plan(self, do_crrej=True):
    ...
    def load_image(self, filt, counter):

        pass


class NICImage:
    path_orig: str = field(default=None)
    path_lv1: str = field(default=None)
    path_lv2: str = field(default=None)
    path_lv3: str = field(default=None)
    path_lv4: str = field(default=None)

    def _load_cal_from_last_hdr(self, key):
        fpath = self.final.header.get(key)
        return None if fpath is None else load_ccd(fpath)

    def __post_init__(self):
        self.orig = load_ccd(self.path_orig)
        self.lv1 = None if self.path_lv1 is None else load_ccd(self.path_lv1)
        self.lv2 = None if self.path_lv2 is None else load_ccd(self.path_lv2)
        self.lv3 = None if self.path_lv3 is None else load_ccd(self.path_lv3)
        self.lv4 = None if self.path_lv4 is None else load_ccd(self.path_lv4)
        self.maxlv = 0
        self.final = self.orig.copy()
        for i in range(4, 0, -1):
            if getattr(self, f"lv{i}") is not None:
                self.maxlv = i
                self.final = getattr(self, f"lv{i}")
                break
        self.mdark = self._load_cal_from_last_hdr("DARKFRM")
        self.mflat = self._load_cal_from_last_hdr("FLATFRM")
        self.mfrin = self._load_cal_from_last_hdr("FRINFRM")
        self.mmask = self._load_cal_from_last_hdr("MASKFILE")


"""
Parameters
----------
inputs : glob pattern, [list of] path-like, optional
    The path to the FITS files to be reduced. Examples:
    ``"_original_32bit/4_Vesta_20191022/*.fits"``, ``["dir/file1.fits",
    "dir/file2.fits"]``, etc.
    Default: ``"*.fits"``

dir_log : path-like, optional
    The directory to save the log files, including header summary and
    reuction plan files.
    Default: ``_logs/`` (will be created if not exist)

dir_dark, dir_frin: path-like, optional
    The directory of **master** dark, fringe. If `None` (default), each
    of it will be searched within the `inputs`, after generation of
    level 1 data. These can be specified if the user wants to use
    different night's dark or fringe.

dir_flat: path-like, optional
    The directory of **master** flat to be used.

dir_mask: path-like, optional
    The directory of **master** mask to be used for bad pixel masking.
    If `None`, no mask will be applied, and it can be dangerous
    especially for the Fourier pattern removal (i.e., making level 1
    data).

Notes
-----
It is intended for NIC science frames, not calibration frames (e.g.,
night to obtain FLAT frames). For those calibration frame data, use
functions in `prepare.py`, `preproc.py`, etc.

Desired usage: Many auxiliary files/directories (including log,
calibration files, etc) are made relative to the current working
directory (``"."``). Thus, it is recommended to put all raw data
folders (e.g., ``nic191210/``, etc) under the same folder
(``./_original_32bit/nic191210/``, etc), and then use this class at the
top path ``.``. All other paths must be relative to ``.``.
